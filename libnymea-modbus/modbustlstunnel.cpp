    ...
    : QObject(parent)
{
    ...
    m_handshakeTimer.setInterval(10000);
    connect(&m_handshakeTimer, &QTimer::timeout, this, [this]() {
        if (!m_sslSocket || m_sslSocket->isEncrypted())
            return;
        emit tunnelError(tr("The TLS handshake timed out."));
        m_sslSocket->abort();
    });
}

void ModbusTlsTunnel::start(const QHostAddress &address,
                            quint16 port,
                            const QString &serverName,
                            const QSslConfiguration &configuration,
                            const QString &acceptedFingerprint)
{
    stop();
    m_stopping = false;
    ...
    connect(m_sslSocket, &QSslSocket::newSessionTicketReceived, this, [this]() {
        const QByteArray ticket = m_sslSocket->sslConfiguration().sessionTicket();
        if (!ticket.isEmpty())
            emit sessionTicketReceived(ticket);
    });
#endif
...
#else
    connect(m_sslSocket,
            QOverload<QAbstractSocket::SocketError>::of(&QSslSocket::error),
            this,
    ...
    m_sslSocket->connectToHostEncrypted(hostName,
                                        port,
                                        serverName.isEmpty() ? hostName : serverName);
...
void ModbusTlsTunnel::createLoopbackServer()
{
    ...
    for (int attempt = 0; attempt < 8 && !listening; ++attempt) {
        const quint32 address = (127u << 24) | (QRandomGenerator::global()->bounded(1u, 255u) << 16)
    ...
    if (!listening) {
        emit tunnelError(
            tr("Could not create the local Modbus TLS bridge: %1").arg(m_server.errorString()));
        m_sslSocket->abort();
        return;
    }
