...
def writeTcpHeaderFile():
    ...
    writeLine(headerFile, '#include <QSet>')
    writeLine(headerFile)
    ...
    writeLine(headerFile, '    bool isIllegalDataAddressError(QModbusReply *reply) const;')
    writeLine(headerFile, '    bool markUnsupportedUpdateRequest(const QString &requestId, const QString &description);')
    ...
    writeLine(headerFile, '    QSet<QString> m_unsupportedUpdateRequests;')
    writeLine(headerFile)
    ...
    if queuedRequests:
        ...
        writeLine(headerFile, '    void enqueueRequest(%s::Function function, const QString &requestId);' % (className))
...
def writeTcpSourceFile():
    ...
    writeLine(sourceFile, '            m_unsupportedUpdateRequests.clear();')
    ...
    writeLine(sourceFile, 'bool %s::isIllegalDataAddressError(QModbusReply *reply) const' % (className))
    writeLine(sourceFile, '{')
    writeLine(sourceFile, '    if (!reply || reply->error() != QModbusDevice::ProtocolError)')
    writeLine(sourceFile, '        return false;')
    writeLine(sourceFile)
    writeLine(sourceFile, '    QModbusResponse response = reply->rawResult();')
    writeLine(sourceFile, '    return response.isException() && response.exceptionCode() == QModbusPdu::IllegalDataAddress;')
    writeLine(sourceFile, '}')
    writeLine(sourceFile)

    writeLine(sourceFile, 'bool %s::markUnsupportedUpdateRequest(const QString &requestId, const QString &description)' % (className))
    writeLine(sourceFile, '{')
    writeLine(sourceFile, '    if (m_unsupportedUpdateRequests.contains(requestId))')
    writeLine(sourceFile, '        return false;')
    writeLine(sourceFile)
    writeLine(sourceFile, '    m_unsupportedUpdateRequests.insert(requestId);')
    writeLine(sourceFile, '    qCWarning(dc%s()) << "Disable unsupported update request" << description << "for this connection after illegal data address response."; ' % (className))
    writeLine(sourceFile, '    return true;')
    writeLine(sourceFile, '}')
    writeLine(sourceFile)
...
def writeRtuHeaderFile():
    ...
    writeLine(headerFile, '    bool isIllegalDataAddressError(ModbusRtuReply *reply) const;')
    writeLine(headerFile, '    bool markUnsupportedUpdateRequest(const QString &requestId, const QString &description);')
    writeLine(headerFile, '    void testReachability();')
    writeLine(headerFile)
...
def writeRtuSourceFile():
    ...
    writeLine(sourceFile, 'bool %s::isIllegalDataAddressError(ModbusRtuReply *reply) const' % (className))
    writeLine(sourceFile, '{')
    writeLine(sourceFile, '    if (!reply || reply->error() == ModbusRtuReply::NoError)')
    writeLine(sourceFile, '        return false;')
    writeLine(sourceFile)
    writeLine(sourceFile, '    return reply->errorString().contains(QStringLiteral("Illegal data address"), Qt::CaseInsensitive);')
    writeLine(sourceFile, '}')
    writeLine(sourceFile)
