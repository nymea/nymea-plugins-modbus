    ...
    for registerDefinition in registerDefinitions:
        ...
        if queuedRequests:
            if 'readSchedule' in registerDefinition and registerDefinition['readSchedule'] == 'init':
                ...
                writeLine(fileDescriptor, '        QPointer<QModbusReply> replyGuard(reply);')
                writeLine(fileDescriptor, '        QTimer::singleShot(0, this, [this, replyGuard, error](){')
                writeLine(fileDescriptor, '            if (!replyGuard)')
                writeLine(fileDescriptor, '                return;')
                writeLine(fileDescriptor)
                writeLine(fileDescriptor, '            QModbusResponse response = replyGuard->rawResult();')
                writeLine(fileDescriptor, '            if (error == QModbusDevice::ProtocolError && response.isException()) {')
                ...
                writeLine(fileDescriptor, '            } else {')
                writeLine(fileDescriptor, '                qCWarning(dc%s()) << "Modbus reply error occurred while updating \\"%s\\" registers from" << m_modbusTcpMaster->hostAddress().toString() << error << replyGuard->errorString();' % (className, registerDefinition['description']))
                writeLine(fileDescriptor, '            }')
                writeLine(fileDescriptor, '        });')
                writeLine(fileDescriptor, '    });')
        else:
    ...
    for blockDefinition in blockDefinitions:
        ...
        if queuedRequests:
            ...
            if 'readSchedule' in blockDefinition and blockDefinition['readSchedule'] == 'init':
                ...
                writeLine(fileDescriptor, '    connect(reply, &QModbusReply::errorOccurred, this, [this, reply] (QModbusDevice::Error error){')
                writeLine(fileDescriptor, '        QPointer<QModbusReply> replyGuard(reply);')
                writeLine(fileDescriptor, '        QTimer::singleShot(0, this, [replyGuard, error](){')
                writeLine(fileDescriptor, '            if (!replyGuard)')
                writeLine(fileDescriptor, '                return;')
                writeLine(fileDescriptor)
                writeLine(fileDescriptor, '            QModbusResponse response = replyGuard->rawResult();')
                writeLine(fileDescriptor, '            if (error == QModbusDevice::ProtocolError && response.isException()) {')
                writeLine(fileDescriptor, '                qCWarning(dc%s()) << "Modbus reply error occurred while updating block \\"%s\\" registers" << error << replyGuard->errorString() << ModbusDataUtils::exceptionCodeToString(response.exceptionCode());' % (className, blockName))
                writeLine(fileDescriptor, '            } else {')
                writeLine(fileDescriptor, '                qCWarning(dc%s()) << "Modbus reply error occurred while updating block \\"%s\\" registers" << error << replyGuard->errorString();' % (className, blockName))
                writeLine(fileDescriptor, '            }')
                writeLine(fileDescriptor, '        });')
                writeLine(fileDescriptor, '    });')
        else:
...
def writeTestReachabilityImplementationsTcp(fileDescriptor, className, registerDefinitions, checkReachableRegister):
    ...
    writeLine(fileDescriptor, '        QPointer<QModbusReply> replyGuard(m_checkReachableReply);')
    writeLine(fileDescriptor, '        QTimer::singleShot(0, this, [this, replyGuard, error](){')
    writeLine(fileDescriptor, '            if (!replyGuard)')
    writeLine(fileDescriptor, '                return;')
    writeLine(fileDescriptor)
    writeLine(fileDescriptor, '            QModbusResponse response = replyGuard->rawResult();')
    writeLine(fileDescriptor, '            if (error == QModbusDevice::ProtocolError && response.isException()) {')
    writeLine(fileDescriptor, '                qCWarning(dc%s()) << "Modbus reply error occurred while verifying reachability by reading \\"%s\\" register" << error << replyGuard->errorString() << ModbusDataUtils::exceptionCodeToString(response.exceptionCode());' % (className, checkReachableRegister['description']))
    ...
    writeLine(fileDescriptor, '            } else {')
    writeLine(fileDescriptor, '                qCDebug(dc%s()) << "Modbus reply error occurred while verifying reachability by reading \\"%s\\" register" << error << replyGuard->errorString();' % (className, checkReachableRegister['description']))
    writeLine(fileDescriptor, '            }')
    writeLine(fileDescriptor, '        });')
    writeLine(fileDescriptor, '    });')
    writeLine(fileDescriptor, '}')
        ...
        if queuedRequests:
            ...
            for registerDefinition in registerDefinitions:
                ...
                if 'readSchedule' in registerDefinition and registerDefinition['readSchedule'] == 'init':
                    ...
                    writeLine(fileDescriptor, '        QPointer<QModbusReply> replyGuard(reply);')
                    writeLine(fileDescriptor, '        QTimer::singleShot(0, m_initObject, [this, replyGuard, error](){')
                    writeLine(fileDescriptor, '            if (!replyGuard)')
                    writeLine(fileDescriptor, '                return;')
                    writeLine(fileDescriptor)
                    writeLine(fileDescriptor, '            QModbusResponse response = replyGuard->rawResult();')
                    writeLine(fileDescriptor, '            if (error == QModbusDevice::ProtocolError && response.isException()) {')
                    writeLine(fileDescriptor, '                qCWarning(dc%s()) << "Modbus reply error occurred while reading \\"%s\\" registers from" << m_modbusTcpMaster->hostAddress().toString() << error << replyGuard->errorString() << ModbusDataUtils::exceptionCodeToString(response.exceptionCode());' % (className, registerDefinition['description']))
                    writeLine(fileDescriptor, '            } else {')
                    writeLine(fileDescriptor, '                qCWarning(dc%s()) << "Modbus reply error occurred while reading \\"%s\\" registers from" << m_modbusTcpMaster->hostAddress().toString() << error << replyGuard->errorString();' % (className, registerDefinition['description']))
                    writeLine(fileDescriptor, '            }')
                    writeLine(fileDescriptor, '        });')
                    writeLine(fileDescriptor, '    });')
            ...
            for blockDefinition in blockDefinitions:
                ...
                if 'readSchedule' in blockDefinition and blockDefinition['readSchedule'] == 'init':
                    ...
                    writeLine(fileDescriptor, '    connect(reply, &QModbusReply::errorOccurred, m_initObject, [this, reply] (QModbusDevice::Error error){')
                    writeLine(fileDescriptor, '        QPointer<QModbusReply> replyGuard(reply);')
                    writeLine(fileDescriptor, '        QTimer::singleShot(0, m_initObject, [replyGuard, error](){')
                    writeLine(fileDescriptor, '            if (!replyGuard)')
                    writeLine(fileDescriptor, '                return;')
                    writeLine(fileDescriptor)
                    writeLine(fileDescriptor, '            QModbusResponse response = replyGuard->rawResult();')
