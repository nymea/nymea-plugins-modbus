...
#include "neuronextension/neuronextensiondiscovery.h"
    ...
    if (thingClassId == digitalInputThingClassId) {
        ...
        foreach (NeuronExtension *neuronExtension, m_neuronExtensions) {
            ...
            foreach (QString circuit, neuronExtension->digitalInputs()) {
                ThingDescriptor descriptor(digitalInputThingClassId, QString("Digital input %1").arg(circuit), QString("Neuron extension %1, slave address %2").arg(neuronExtension->type()).arg(QString::number(neuronExtension->slaveAddress())), parentId);
        ...
        if (!myThings().filterByThingClassId(neuronThingClassId).isEmpty()) {
            ...
            foreach (QString circuit, m_neuron->digitalInputs()) {
                ThingDescriptor descriptor(digitalInputThingClassId, QString("Digital input %1").arg(circuit), QString("Neuron %1").arg(myThings().findById(parentId)->stateValue(neuronThingModelParamTypeId).toString()), parentId);
                foreach (Thing *thing, myThings().filterByParentId(parentId)) {
                    if (thing->paramValue(digitalInputThingCircuitParamTypeId) == circuit) {
        ...
        foreach (auto neuronExtension, m_neuronExtensions) {
            ThingId parentId = m_neuronExtensions.key(neuronExtension);
            foreach (QString circuit, neuronExtension->relayOutputs()) {
                ThingDescriptor descriptor(relayOutputThingClassId, QString("Relay output %1").arg(circuit), QString("Neuron extension %1, Slave address %2").arg(neuronExtension->type()).arg(QString::number(neuronExtension->slaveAddress())), parentId);
                foreach(Thing *thing, myThings().filterByParentId(parentId)) {
        ...
        foreach (NeuronExtension *neuronExtension, m_neuronExtensions) {
            ...
            foreach (QString circuit, neuronExtension->digitalOutputs()) {
                ThingDescriptor descriptor(digitalOutputThingClassId, QString("Digital output %1").arg(circuit), QString("Neuron extension %1, Slave address %2").arg(neuronExtension->type()).arg(QString::number(neuronExtension->slaveAddress())), parentId);
                foreach(Thing *thing, myThings().filterByParentId(parentId)) {
                    if (thing->paramValue(digitalOutputThingCircuitParamTypeId) == circuit) {
                        qCDebug(dcUniPi()) << "Found already added Circuit:" << circuit << parentId;
                        descriptor.setThingId(thing->id());
        ...
        if (!myThings().filterByThingClassId(neuronThingClassId).isEmpty()) {
            ...
            foreach (QString circuit, m_neuron->analogInputs()) {
                ThingDescriptor descriptor(analogInputThingClassId, QString("Analog input %1").arg(circuit), QString("Neuron %1").arg(myThings().findById(parentId)->stateValue(neuronThingModelParamTypeId).toString()), parentId);
                foreach(Thing *thing, myThings().filterByParentId(parentId)) {
                    if (thing->paramValue(analogInputThingCircuitParamTypeId) == circuit) {
                        qCDebug(dcUniPi()) << "Found already added Circuit:" << circuit << parentId;
                        descriptor.setThingId(thing->id());
        ...
        foreach (NeuronExtension *neuronExtension, m_neuronExtensions) {
            ...
            foreach (QString circuit, neuronExtension->analogOutputs()) {
                ThingDescriptor descriptor(analogOutputThingClassId, QString("Analog output %1").arg(circuit), QString("Neuron extension %1, Slave address %2").arg(neuronExtension->type()).arg(QString::number(neuronExtension->slaveAddress())), parentId);
                foreach(Thing *thing, myThings().filterByParentId(parentId)) {
                    if (thing->paramValue(analogOutputThingCircuitParamTypeId) == circuit) {
                        qCDebug(dcUniPi()) << "Found already added Circuit:" << circuit << parentId;
                        descriptor.setThingId(thing->id());
        ...
        if (!myThings().filterByThingClassId(neuronThingClassId).isEmpty()) {
            ...
            foreach (QString circuit, m_neuron->analogOutputs()) {
                ThingDescriptor descriptor(analogOutputThingClassId, QString("Analog output %1").arg(circuit), QString("Neuron %1").arg(myThings().findById(parentId)->stateValue(neuronThingModelParamTypeId).toString()), parentId);
                foreach(Thing *thing, myThings().filterByParentId(parentId)) {
                    if (thing->paramValue(analogOutputThingCircuitParamTypeId) == circuit) {
                        qCDebug(dcUniPi()) << "Found already added Circuit:" << circuit << parentId;
                        descriptor.setThingId(thing->id());
        ...
        for (NeuronExtensionBus *neuronExtensionBus: m_neuronExtensionBus.values()) {
            ...
            auto extensionDiscovery = new NeuronExtensionDiscovery(neuronExtensionBus);
...
void IntegrationPluginUniPi::setupThing(ThingSetupInfo *info)
{
    ...
    if(thing->thingClassId() == uniPi1ThingClassId ||
        ...
        if (m_unipi) {
            ...
            return info->finish(Thing::ThingErrorSetupFailed, QT_TR_NOOP("There is already a UniPi gateway in the system."));
        }

        const auto type = (thing->thingClassId() == uniPi1ThingClassId) ? UniPi::UniPiType::UniPi1 : UniPi::UniPiType::UniPi1Lite;
        m_unipi = new UniPi(hardwareManager()->i2cManager(), type, this);
        ...
        const QString port = thing->paramValue(neuronExtensionBusThingSerialPortParamTypeId).toString();
        const int baudrate = thing->paramValue(neuronExtensionBusThingBaudrateParamTypeId).toInt();
        const QString parity = thing->paramValue(neuronExtensionBusThingParityParamTypeId).toString();
        ...
        const int slaveAddress = thing->paramValue(neuronExtensionThingSlaveAddressParamTypeId).toInt();
        const ThingId neuronExtensionBusThingId = ThingId(thing->paramValue(neuronExtensionThingNeuronExtensionBusUuidParamTypeId).toUuid());
        const QString modelString = thing->paramValue(neuronExtensionThingModelParamTypeId).toString();
        ...
        auto bus = m_neuronExtensionBus.value(neuronExtensionBusThingId);
        auto neuronExtension = new NeuronExtension(type, bus, slaveAddress, this);
...
void IntegrationPluginUniPi::postSetupThing(Thing *thing)
{
    ...
    if (thing->thingClassId() == neuronThingClassId) {
        if(!m_neuron)  {
            qCWarning(dcUniPi()) << "Neuron object not available:" << thing->name();
            return;
        }
        ...
        auto neuronExtension = m_neuronExtensions.value(thing->id());
        if (!neuronExtension) {
            return;
        }
        ...
        if (parentThing->thingClassId() == neuronThingClassId) {
            if (!m_neuron) {
                qCWarning(dcUniPi()) << "Neuron extension object not available:" << thing->name();
                return;
            }
...
void IntegrationPluginUniPi::thingRemoved(Thing *thing)
{
    ...
    if(m_neuronExtensions.contains(thing->id())) {
        m_neuronExtensions.take(thing->id())->deleteLater();
    } else if (m_neuronExtensionBus.contains(thing->id())) {
        m_neuronExtensionBus.take(thing->id())->deleteLater();
    } else if (thing->thingClassId() == neuronThingClassId) {
        m_neuron->requestInterruption();
        if (!m_neuron->wait(3000)) {
...
{
    auto neuronExtension = static_cast<NeuronExtension *>(sender());
...
{
    auto neuron = static_cast<NeuronExtension *>(sender());
...
{
    foreach(Thing *thing, myThings().filterByThingClassId(digitalInputThingClassId)) {
        if (thing->paramValue(digitalInputThingCircuitParamTypeId).toString() == circuit) {
...
{
    foreach(Thing *thing, myThings().filterByThingClassId(digitalOutputThingClassId)) {
        if (thing->paramValue(digitalOutputThingCircuitParamTypeId).toString() == circuit) {
            thing->setStateValue(digitalOutputPowerStateTypeId, value);
            return;
        }
...
{
    foreach(Thing *thing, myThings().filterByThingClassId(analogInputThingClassId)) {
        if (thing->paramValue(analogInputThingCircuitParamTypeId).toString() == circuit) {
            thing->setStateValue(analogInputInputValueStateTypeId, value);
            thing->setStateValue(analogInputPercentageStateTypeId, value*10);
            return;
...
{
    foreach(Thing *thing, myThings().filterByThingClassId(analogOutputThingClassId)) {
        thing->setStateValue(analogOutputOutputValueStateTypeId, value);
        thing->setStateValue(analogOutputPercentageStateTypeId, value*10);
        return;
    }
