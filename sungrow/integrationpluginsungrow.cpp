    ...
    if (thing->thingClassId() == sungrowInverterTcpThingClassId) {
        ...
        connect(sungrowConnection, &SungrowModbusTcpConnection::reachableChanged, thing, [this, thing, sungrowConnection](bool reachable){
            ...
            if (reachable) {
                ...
                if (child) {
                    ...
                    child->setStateValue(sungrowBatteryBatteryLevelStateTypeId, 0);
                    child->setStateValue(sungrowBatteryBatteryCriticalStateTypeId, true);
        ...
        connect(sungrowConnection, &SungrowModbusTcpConnection::updateFinished, thing, [=](){
            ...
            if (meterThing) {
                ...
                qCInfo(dcSungrow()) << "Power generated from PV:" << (runningState & (0x1 << 0) ? "true" : "false");
                qCInfo(dcSungrow()) << "Battery charging:" << (runningState & (0x1 << 1) ? "true" : "false");
                qCInfo(dcSungrow()) << "Battery discharging:" << (runningState & (0x1 << 2) ? "true" : "false");
                qCInfo(dcSungrow()) << "Positive load power:" << (runningState & (0x1 << 3) ? "true" : "false");
                qCInfo(dcSungrow()) << "Feed-in power:" << (runningState & (0x1 << 4) ? "true" : "false");
                qCInfo(dcSungrow()) << "Import power from grid:" << (runningState & (0x1 << 5) ? "true" : "false");
                qCInfo(dcSungrow()) << "Negative load power:" << (runningState & (0x1 << 7) ? "true" : "false");
