...
{
    if (!m_connection->connected() && !m_initFinishedSuccess) {
        qCDebug(dcSunSpec()) << "SolarEdgeBattery: Not reading block data. The modbus connection is not available.";
        m_timer.stop();
        emit initFinished(false);
        return;
    }
    ...
    QModbusReply *reply = m_connection->modbusTcpClient()->sendReadRequest(request, m_connection->slaveId());
    if (!reply) {
        qCWarning(dcSunSpec()) << "SolarEdgeBattery: Error occurred while reading data from" << m_connection->hostAddress().toString() << m_connection->modbusTcpClient()->errorString();
        return;
    }
    ...
    connect(reply, &QModbusReply::errorOccurred, this, [] (QModbusDevice::Error error) {
        qCWarning(dcSunSpec()) << "SolarEdgeBattery: Modbus reply error occurred:" << error;
    });
